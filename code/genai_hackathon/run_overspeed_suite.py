# genai_hackathon/run_overspeed_suite.py
...
OUT = "genai_hackathon/overspeed_suite_out"
...
def overspeed_spec(vt_trace, sp_trace, tol=0.03):
    ...
    Positive rho means no overspeed.
    """
    vt_trace = np.asarray(vt_trace, dtype=float)
    sp_trace = np.asarray(sp_trace, dtype=float)

    limit = sp_trace * (1.0 + tol)
    margin = (limit - vt_trace) / np.maximum(sp_trace, 1e-6)

    rho = float(np.min(margin))
    return rho >= 0.0, rho
...
def rollout_ppo(env, model, seed, tol):
    ...
    sat, rho = overspeed_spec(vt, sp_hist, tol=tol)
...
def rollout_conf(env, model, seed, tol):
    obs, info = env.reset(seed=seed)
    base = _base_env(env)
...
def rollout_genai(env, model, seed, tol):
    obs, info = env.reset(seed=seed)
    base = _base_env(env)
...
def evaluate_method(method, model, tol, n=N):
    sats, rhos = [], []
    ...
    for i in range(n):
        ...
        if method == "PPO":
            res = rollout_ppo(env, model, seed, tol)
        elif method == "PPO+CONF":
            res = rollout_conf(env, model, seed, tol)
        elif method == "PPO+GENAI":
            res = rollout_genai(env, model, seed, tol)
        else:
...
def main():
    ...
    print("\n=== Overspeed Safety Suite ===")
    print(f"{'Method':10s} | {'Sat%':>6s} | {'Mean rho':>9s} | {'mean u':>7s}")
    print("-" * 46)

    for method in methods:
        res = evaluate_method(method, model, tol=tol, n=N)

        print(
            f"{method:10s} | {100*res['sat']:6.1f} | "
            f"{res['rho']:9.4f} | {res['mean_u']:7.4f}"
        )
    ...
    with open(os.path.join(OUT, "overspeed_results.csv"), "w", newline="", encoding="utf-8-sig") as f:
        w = csv.DictWriter(
            f,
            fieldnames=[
                "method",
    ...
    with open(os.path.join(OUT, "overspeed_results.json"), "w", encoding="utf-8") as f:
        json.dump(rows, f, indent=2)

    print("\nSaved:")
    print(f" - {os.path.join(OUT, 'overspeed_results.csv')}")
    print(f" - {os.path.join(OUT, 'overspeed_results.json')}")
