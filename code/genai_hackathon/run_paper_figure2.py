...
OUT = "genai_hackathon/paper_figures_nominal_stress2"
...
def compute_metrics(vt, sp, u, changed, warmup_s=10.0):
    ...
    err = (vt_eval - sp_eval) / np.maximum(sp_eval, 1e-6)
    abs_err = np.abs(err)

    rmse = float(np.sqrt(np.mean(abs_err ** 2)))
    mae = float(np.mean(abs_err))
    overshoot = float(100.0 * np.max((vt_eval - sp_eval) / np.maximum(sp_eval, 1e-6)))

    upper = (sp_eval * (1.0 + TOL_OVER) - vt_eval) / np.maximum(sp_eval, 1e-6)
    lower = (vt_eval - sp_eval * (1.0 - TOL_UNDER)) / np.maximum(sp_eval, 1e-6)

    rho = float(np.min(np.minimum(upper, lower)))
    sat = int(rho >= 0.0)

    inside = (vt_eval >= sp_eval * (1.0 - TOL_UNDER)) & (vt_eval <= sp_eval * (1.0 + TOL_OVER))
